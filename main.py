...
class MainApp(App):
    ...
    def carregar_infos_usuario(self):
        try:
            ...
            requisicao = requests.get(
                f'https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}')
    ...
    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            mensagem_texto.text = "Vendedor não pode se incluir à equipe "
        else:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f'https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}',
                               data=info)
    ...
    def selecionar_cliente(self, foto, *args):
        # pintar todas as letras de branco
        pagina_adicionarvendas = self.root.ids["adicionarvendaspage"]
        ...
        for item in list(lista_clientes.children):
            item.color = (1, 1, 1, 1)
        # pintar de azul o item selecionado e de branco o restante
    ...
    def selecionar_unidade(self, id_label, *args):
        pagina_selecionarvendas = self.root.ids["adicionarvendaspage"]
        self.unidade = id_label.replace("unidades_", "")
        pagina_selecionarvendas.ids["unidades_kg"].color = (1, 1, 1, 1)
        pagina_selecionarvendas.ids["unidades_unidades"].color = (1, 1, 1, 1)
        pagina_selecionarvendas.ids["unidades_litros"].color = (1, 1, 1, 1)

        # pintar o selecionado de azul
        pagina_selecionarvendas.ids[id_label].color = (0, 207/255, 219/255, 1)
    ...
    def adicionar_venda(self):
        ...
        if not quantidade:
            ...
            try:
                ...
                agina_adicionarvendas.ids["label_quantidade"].color = (1, 0, 0, 1)
        # dado que ele preencheu tudo, vamos executar o codigo de executar venda
        if (cliente and
                produto and
                unidade and
                preco and
            ...
            link = f"https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}"

            requests.post(link, data=info)

            banner = BannerVenda(cliente=cliente, produto=produto, foto_cliente=foto_cliente,
                                 foto_produto=foto_produto, data=data, preco=preco,
            ...
            requisicao = requests.get(f"https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}")
    ...
    def carregar_todas_vendas(self):
        ...
        link = f'https://aplicativovendashash-dec2c-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"'
        requisicao = requests.get(link)
        requisicao_dic = requisicao.json()
